...
#define AC10U_BUTTONS_POLL_INTERVAL		100
#define AC10U_BUTTONS_DEBOUNCE_INTERVAL	(3 * AC10U_BUTTONS_POLL_INTERVAL)
...
static struct gpio_keys_button ac10u_buttons[] __initdata = {
	{
		...
		.type		= EV_KEY,
		.code		= KEY_RESTART,
		.debounce_interval = AC10U_BUTTONS_DEBOUNCE_INTERVAL,
		//.threshold	= 3,
...
struct gpio ac10u_phy_reset_pin_data = {
	.gpio		= BSP_GPIO_PIN_H2,
	.flags          = GPIOF_ACTIVE_LOW,
	.label		= "GPIO_H2",
};

static struct platform_device __initdata *ac10u_devs[] = {
...
static void __init ac10u_setup(void)
{
	int i;


	rtl819x_phy_reset_pin.dev.platform_data = &ac10u_phy_reset_pin_data;

	platform_add_devices(ac10u_devs, ARRAY_SIZE(ac10u_devs));
	
	rtl819x_register_leds_gpio(-1, ARRAY_SIZE(ac10u_leds_gpio),
		ac10u_leds_gpio);

	for (i=0; i<ARRAY_SIZE(ac10u_leds_gpio); i++) {
		rtl819x_gpio_pin_enable(ac10u_leds_gpio[i].gpio);
	}

	for (i=0; i<ARRAY_SIZE(ac10u_buttons); i++) {
		rtl819x_gpio_pin_enable(ac10u_buttons[i].gpio);
	}
	rtl819x_add_device_gpio_buttons(-1, AC10U_BUTTONS_POLL_INTERVAL,
				       ARRAY_SIZE(ac10u_buttons),
				       ac10u_buttons);

//	ac10u_set_sd_pinmux();
}

MIPS_MACHINE(RTL8197_MACH_AC10U, "AC10U", "TENDA AC10U",
			 ac10u_setup);
