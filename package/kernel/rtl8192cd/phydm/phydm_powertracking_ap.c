...
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	u1Byte			i = 0, CckBBswingMask = 0;
	u4Byte			CckbbSwing = 0;
	u4Byte			CckswingTableSize = 0;
	pu4Byte			pCckSwingTable = 0;
	prtl8192cd_priv	priv = pDM_Odm->priv;

#if (RTL8197F_SUPPORT == 1)
	if (GET_CHIP_VER(priv) == VERSION_8197F) {
		CckbbSwing = PHY_QueryBBReg(priv, 0xab4, 0X7ff);
		pCckSwingTable = CCKSwingTable_Ch1_Ch14_8723D;
		CckswingTableSize = CCK_TABLE_SIZE_8723D;
		CckBBswingMask = 0;
	}
#endif

	for (i = 0; i < CckswingTableSize - 1; i++) {
		u4Byte tableValue = pCckSwingTable[i] >> CckBBswingMask;

		if (CckbbSwing == tableValue)
			break;
	}

	ODM_RT_TRACE(pDM_Odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("CckbbSwing=0x%x Cckbbswing_index=%d\n", CckbbSwing, i));
	return i;
}
	...
	)
{
...
#if (RTL8197F_SUPPORT == 1)
	if (GET_CHIP_VER(priv) == VERSION_8197F)
		defaultCckSwingIndex = getCckSwingIndex(pDM_Odm);
#endif
	...
	if (GET_CHIP_VER(priv) == VERSION_8197F) {
		...
		pRFCalibrateInfo->DefaultCckIndex = (defaultCckSwingIndex >= (CCK_TABLE_SIZE_8723D - 1)) ? 28 : defaultCckSwingIndex;
	}
#endif
