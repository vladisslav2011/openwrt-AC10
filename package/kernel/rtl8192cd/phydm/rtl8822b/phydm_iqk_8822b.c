	...
	)
{
	...
	ODM_SetRFReg(p_dm_odm, (ODM_RF_RADIO_PATH_E)path, add, bRFRegOffsetMask, data);

	for (i = 0; i < 100; i++) {
		if (ODM_GetRFReg(p_dm_odm, (ODM_RF_RADIO_PATH_E)path, add, bRFRegOffsetMask) == data)
			break;
		else {
			ODM_delay_us(10);
			ODM_SetRFReg(p_dm_odm, (ODM_RF_RADIO_PATH_E)path, add, bRFRegOffsetMask, data);
...
/*		ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("[IQK]AFE setting for Normal mode!!!!\n")); */
	...
	ODM_SetBBReg(pDM_Odm, 0x9a4, BIT31, 0x0);
}
...
{	
	...
	/*0xdf[4]=0*/
	_iqk_rf_set_check(pDM_Odm, ODM_RF_PATH_A, 0xdf, RF_backup[0][ODM_RF_PATH_A] & (~BIT(4)));
	_iqk_rf_set_check(pDM_Odm, ODM_RF_PATH_B, 0xdf, RF_backup[0][ODM_RF_PATH_B] & (~BIT(4)));
...
{	
	...
	for (path = 0; path < 2; path++) {
		...
		tmp = ODM_GetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0xdf, bRFRegOffsetMask) | BIT(4);
		_iqk_rf_set_check(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0xdf, tmp);
...
/*	ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("[IQK]Set MACBB setting for IQK!!!!\n"));*/
	/*disable PMAC*/
	ODM_SetBBReg(pDM_Odm, 0xb00, BIT8, 0x0);
}
